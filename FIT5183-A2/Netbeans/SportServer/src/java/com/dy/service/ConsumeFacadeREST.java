...
public class ConsumeFacadeREST extends AbstractFacade<Consume> {
    ...
    public void create(Consume entity) {
        ...
        System.out.println("time"+entity.getTime()+",step"+entity.getSteps());
...
//        System.out.println("steps * constant =  "+res);
    ...
    @Produces({ MediaType.APPLICATION_JSON})
    public List<Consume> findByUserIdDate(@PathParam("userId") String userId,@PathParam("date") String date) {
    ...
    @GET
    @Path("undateSteps/{id}/{steps}/{date}")
    @Produces({MediaType.APPLICATION_JSON})
    public String undateSteps(@PathParam("id") String id, @PathParam("steps") String steps,@PathParam("date") String date) {
        List<Consume> list = this.findByUserIdDate(id, date);
        if (list.size() != 0) {
            Consume c = list.get(0);
            c.setSteps(Integer.valueOf(steps));
            edit(c.getId(), c);

        } else {
            Consume c = new Consume();
            c.setUserId(userFacadeREST.find(id));
            c.setSteps(Integer.valueOf(steps));
            this.create(c);
        }
