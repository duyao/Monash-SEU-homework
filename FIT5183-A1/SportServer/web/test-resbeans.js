...
function TestSupport() {
    this.wadlDoc = null;
    this.wadlURL = '';
    this.wadlErr = '无法访问 WADL。请重新启动 REST 风格的应用程序并刷新此页, 或者在此处提供 URL。这可能是由于 REST 实现未自动提供 WADL 文件 (例如 JBoss RESTEasy) 所造成的。在这种情况下, 您必须预先手动生成它或者切换到 Jersey 库。';
...
TestSupport.prototype = {    
    ...
    initFromWadl : function () {
        ...
        if(wDoc != null) {
            ...
            this.updatepage('content', '<span class=bld>帮助页面</span><br/><br/><p>无法访问 WADL: 请重新启动 REST 应用程序, 然后刷新此页。这可能是由于 REST 实现未自动提供 WADL 文件 (例如 JBoss RESTEasy) 所造成的。在这种情况下, 您必须预先手动生成它或者切换到 Jersey 库。</p><p>如果仍然出现此错误, 并且您使用带有 Firebug 插件的 Firefox 访问此页面, <br/>则需要对本地文件禁用 Firebug。该插件位于 Firefox 菜单栏中, 请选中<br/>“工具 > Firebug > 对本地文件禁用 Firebug”</p>');
        }            
    },
